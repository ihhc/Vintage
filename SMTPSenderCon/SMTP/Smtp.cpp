...
void CSmtpMessage::CommitHeaders()
{
  ...
  strValue  = _T("News Delivery Mailer");
  if (Headers.FindKey(strHeader) == -1) Headers.Add(strHeader,strValue);
  ...
  WORD wLang = MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US);
  DWORD dwLocale = MAKELCID(wLang,SORT_DEFAULT);
  GetDateFormat(dwLocale,0,&Timestamp,_T("ddd, d MMM yyyy"),szDate,64);
  GetTimeFormat(dwLocale,0,&Timestamp,_T("H:mm:ss"),szTime,64);

  // Add the date/time stamp to the message headers
  wsprintf(szOut,_T("%s %s +%4.4d (MSD)"),szDate,szTime,GMTOffset);
  strHeader = _T("Date");
  strValue = szOut;
...
int CSmtp::SendData(CSmtpMessage &msg)
{
  ...
  // Catastrophic failure, usually a user-abort.  Terminate the session
  if (nRet == -1) Close();
